function [u_new,v_new,p_new,F1_new,F2_new,LHS,A,B1,B2] = IMPLICIT_PRESCRIBED_MOTION(u,v,p,F1,F2,dt,dx,dy,N,NIB,mu,rho,mfac,x,y,X)
...
%Edit the matrix to impose dirichlet boundary conditions
Lapv_x(1,1) = -4/(dx*dx);
...
% Need to interpolate u and v at the grid corners to use compact centered
% difference stencil.
...
S_x = zeros(N*(N-1),2*NIB); %Spreading operator in x
S_y = zeros(N*(N-1),2*NIB); %Spreading operator in y
S_x1 = zeros(2*NIB,N*(N-1)); % Interp operator in x
S_y1 = zeros(2*NIB,N*(N-1)); %Interp operator in y 
N4 = zeros((N*N),2*NIB);
N5 = zeros(2*NIB,N*N);
...
LHS = [0.*A1 - Lapu, N1, G_x, S_x; N2, 0.*A2 - Lapv, G_y, S_y; -Dx, -Dy,N3,N4;S_x1,S_y1,N5,N6];
...
for k = 1:NIB
    ...
    sx = [sxx,sxy];
    syy = X(k,2)/dy;
    syx = X(k,1)/dx + 1/2; %subtract off 1/2 because we are using the mac grid
    sy = [syx,syy];
    %Get the relevant indicies for the forcing terms
    Ixx(k,:) = floor(sx); 
    Iyy(k,:) = floor(sy);
    rx(k,:) = (sx - Ixx(k,:));
    ry(k,:) = (sy - Iyy(k,:));
    %Get the Eulerian indices which interact with the delta function at
    %this time step. This part of the code is dependent on which IB kernel
    %is being used.... I need to try and generalize this a bit.
    %for the IB4 kernel...
%     j1x(k,:) = (Ixx(k,1)-1):(Ixx(k,1)+2); 
%     i1x(k,:) = (Ixx(k,2)-1):(Ixx(k,2)+2);
%     j1y(k,:) = (Iyy(k,1)-1):(Iyy(k,1)+2);
%     i1y(k,:) = (Iyy(k,2)-1):(Iyy(k,2)+2);

%The following is for the Bsline2 kernel in both the x and y directions ---> need to figure out a way
%around this if statements
% if rx(k,1) < 0.5
%     j1x(k,:) = (Ixx(k,1)-1):(Ixx(k,1) + 1);
% else
%     j1x(k,:) = (Ixx(k,1)):(Ixx(k,1)+2);
% end
% 
% if rx(k,2) < 0.5
%     i1x(k,:) = (Ixx(k,2)-1):(Ixx(k,2)+1);
% else
%     i1x(k,:) = Ixx(k,2):(Ixx(k,2)+2);
% end
% 
% if ry(k,1) < 0.5
%     j1y(k,:) = (Iyy(k,1)-1):(Iyy(k,1) + 1);
% else
%     j1y(k,:) = (Iyy(k,1)):(Iyy(k,1)+2);
% end
% 
% if ry(k,2) < 0.5
%     i1y(k,:) = (Iyy(k,2)-1):(Iyy(k,2) + 1);
% else
%     i1y(k,:) = (Iyy(k,2)):(Iyy(k,2)+2);
% end

% Evaluate the IB kernel at the relevant locations (plug in the desired
% IB kernels here, for now IB_4 is used).
%     wsx(:,:,k) = IB4_1(rx(k,1)).*IB4_2(rx(k,2));
%     wsy(:,:,k) = IB4_1(ry(k,1)).*IB4_2(ry(k,2));

% Try out the Bspline2 functions
%     wsx(:,:,k) = kron(BS_2x(rx(k,1)),BS_2y(rx(k,2)));
%     wsy(:,:,k) = kron(BS_2x(ry(k,1)),BS_2y(ry(k,2)));

% Try out the interpolation scheme proposed in https://www.sciencedirect.com/science/article/pii/S0021999122005629?via%3Dihub
% It is a combination of the BS3 and the Bs2 kernel functions
j1x(k,:) =  (Ixx(k,1)-1):(Ixx(k,1)+2); 
if rx(k,2) < 0.5
    i1x(k,:) = (Ixx(k,2)-1):(Ixx(k,2)+1);
else
...
wsx(:,:,k) = BS_3x(rx(k,1)).*BS_2y(rx(k,2));
wsy(:,:,k) = BS_2x(ry(k,1)).*BS_3y(ry(k,2));
    
    
    %Fill in matrix entries corresponding to the relevant forcing terms
    ...
    for q = 1:length(j1y(k,:))
        for n = 1:length(i1y(k,:))
    ...
    LHS(N*(N-1) + N*(N-1) + N*N + k,(j1x(k,q)-2)*N + i1x(k,n)) = ...
        LHS(N*(N-1) + N*(N-1) + N*N + k,(j1x(k,q)-2)*N + i1x(k,n)) + wsx(q,n,k);
        end
    end
    for q = 1:length(j1y(k,:))
        for n = 1:length(i1y(k,:))
    LHS(N*(N-1) + N*(N-1) + N*N + NIB + k, N*(N-1) + (j1y(k,q)-1)*(N-1) + (i1y(k,n)-1)) = ...
        LHS(N*(N-1) + N*(N-1) + N*N + NIB + k, N*(N-1) + (j1y(k,q)-1)*(N-1) + (i1y(k,n)-1)) - wsy(q,n,k);
...
RHS = [0.*A1*u_int(:) + 0.*Conv_u_rhs(:) +  bcu_in/(dx*dx) + bcu_out/(dx*dx);...
    0.*A2*v_int(:) + 0.*Conv_v_rhs(:) + bcv_bot/(dy*dy) + bcv_top/(dy*dy);...
