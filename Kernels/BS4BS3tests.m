% Tests for the BS4BS3 composite kernel
...
ry = sy-Iyy;
      ...
      i1x = mod(Ixx(2)-1:Ixx(2)+2,N)+1;
   if rx(1) < 0.5
       j1x = mod(Ixx(1)-2:Ixx(1)+2,N)+1;
   else
       j1x = mod(Ixx(1)-1:Ixx(1)+3,N)+1;
   end
   
   if ry(2) < 0.5
       i1y = mod(Iyy(2)-2:Iyy(2)+2,N)+1;
   else
       i1y = mod(Iyy(2)-1:Iyy(2)+3,N)+1;
   end
   
   j1y = mod(Iyy(1)-1:Iyy(1)+2,N)+1;
   %get the weights
